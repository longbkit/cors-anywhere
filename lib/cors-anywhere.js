...
function withCORS(headers, request) {
  ...
  if (request.headers['access-control-request-method']) {
    headers['access-control-allow-methods'] = request.headers['access-control-request-method'] + ', POST';
...
function proxyRequest(req, res, proxy) {
  ...
  console.log('Proxying request:', req.method, req.url, 'location.origin:', location.origin);
  console.log('Incoming request headers:', JSON.stringify(req.headers, null, 2));
  // const host = (location.host.startsWith('https://') || location.host.startsWith('http://')) ? location.host : `https://${location.host}`;
  var proxyOptions = {
    changeOrigin: false,
    prependPath: false,
    target: location,
    ...
    buffer: {
      pipe: function(proxyReq) {
        console.log('Outgoing proxy request headers:', JSON.stringify(proxyReq.getHeaders(), null, 2));
        ...
        if (req.method === 'POST') {
          ...
          req.on('end', () => {
            console.log('POST body:', body, 'Content-Length', Buffer.byteLength(body));
        ...
        console.log('proxyReq headers:', JSON.stringify(proxyReq.getHeaders(), null, 2));
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    console.log('handleInitialRequest done', location);
